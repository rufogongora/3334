...
void load(string file)
{
	...
	for (i++; i < programName.size() + 2 + 6; i++)
		startingAddr += line[i];

	// save starting location to be used when executing
	PROGSTART = (unsigned long)strtol(startingAddr.c_str(), NULL, 16);

	// get program length
	for (i; i < programName.size() + 2 + 12; i++)
		programLength += line[i];

	PROGEND = PROGSTART + strtol(programLength.c_str(), NULL, 16);
	...
	while (line[0] != 'E')
	{
		...
		address = strtol(recordAddr.c_str(), NULL, 16);
		for (n = 9; n < line.size(); n += 2)
		{
			string hexBytes = "";
			hexBytes += line[n];
			hexBytes += line[n + 1];
			BYTE tempByte = strtol(hexBytes.c_str(), NULL, 16);

			PutMem(address, &tempByte, 0);
